...
class ImpulseDomainModelTest {
    ...
    fun lokalisierungLiefertTextUndFaelltSonstAufOgZurueck() {
        val lokalisierung = lokalisierung(
            ...
            translations = setOf(
                ...
                Translation(sprache = Sprache.DE, text = "Impulse"),
            ),
        )

        assertEquals("Impulse", lokalisierung.text(Sprache.DE))
    ...
    fun lokalisierungBenoetigtOgTranslation() {
        assertThrows(IllegalArgumentException::class.java) {
            Lokalisierung(
                id = 4,
                translationen = setOf(Translation(sprache = Sprache.DE, text = "Impulse")),
